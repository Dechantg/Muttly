...
const NewsFeedPublicPage = () => {
  ...
  useEffect(() => {
    const fetchMostPopularImages = async () => {
      try {
        const response = await fetch(`${ process.env.NODE_ENV === 'production' ? process.env.REACT_APP_API_BASE_URL : 'api' }/mostliked`);
    ...
    const fetchRecentlyGeneratedImages = async () => {
      try {
        const response = await fetch(`${ process.env.NODE_ENV === 'production' ? process.env.REACT_APP_API_BASE_URL : 'api' }/mostrecent`);
        const data = await response.json();
